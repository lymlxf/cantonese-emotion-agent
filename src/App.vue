...
const assistantState = computed(() => {
  ...
  if (serviceState.value === 'backend-offline') return 'API offline'
  if (serviceState.value === 'ai-offline') return 'AI offline'
  if (serviceState.value === 'demo') return 'Demo sample'
  return 'Frontend ready'
})
const isServiceOffline = computed(() => serviceState.value === 'backend-offline' || serviceState.value === 'ai-offline')
...
const handlePlayback = (msg) => {
  ...
  const audioSource = playableAudioUrl(msg)
  ...
  // 3. 如果是播放“AI 回复”
  else if (msg.isAi && msg.text?.trim()) {
